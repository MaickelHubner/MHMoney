...
/* PrÇ-processadores */
&GLOBAL-DEFINE Table         moeda
&GLOBAL-DEFINE Filho         cotacao
&GLOBAL-DEFINE LabelFilho    Cotaá‰es
&GLOBAL-DEFINE ParentFields  moeda.cd-moeda moeda.ds-moeda
&GLOBAL-DEFINE KeyFields     moeda.cd-moeda
&GLOBAL-DEFINE CamposFilho   cotacao.dt-ini ~
                             cotacao.dt-end ~
                             cotacao.valor
...
&Scoped-Define ENABLED-FIELDS moeda.cd-moeda moeda.ds-moeda 
&Scoped-define ENABLED-TABLES moeda
&Scoped-define FIRST-ENABLED-TABLE moeda
...
&Scoped-Define DISPLAYED-FIELDS moeda.cd-moeda moeda.ds-moeda 
&Scoped-define DISPLAYED-TABLES moeda
&Scoped-define FIRST-DISPLAYED-TABLE moeda
&Scoped-Define DISPLAYED-OBJECTS dt-min vl-min dt-max vl-max 
...
DEFINE VARIABLE dt-max AS DATE FORMAT "99/99/9999":U INITIAL 01/01/001 
     VIEW-AS FILL-IN 
     SIZE 10 BY .79 NO-UNDO.

DEFINE VARIABLE dt-min AS DATE FORMAT "99/99/9999":U INITIAL 01/01/001 
     VIEW-AS FILL-IN 
     SIZE 10 BY .79 NO-UNDO.

DEFINE VARIABLE vl-max AS DECIMAL FORMAT "->>,>>9.999999":U INITIAL 0 
     LABEL "M†ximo" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .79 NO-UNDO.

DEFINE VARIABLE vl-min AS DECIMAL FORMAT "->>,>>9.999999":U INITIAL 0 
     LABEL "M°nimo" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .79 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     dt-min AT ROW 15.25 COL 52 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     vl-min AT ROW 15.25 COL 40 COLON-ALIGNED WIDGET-ID 2
     ...
     moeda.cd-moeda AT ROW 4.5 COL 21 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 5 BY .79
     moeda.ds-moeda AT ROW 4.5 COL 26.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 49 BY .79
     btGoTo AT ROW 1.5 COL 29
     btZoom AT ROW 1.5 COL 35.14
     dt-max AT ROW 15.25 COL 82 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     vl-max AT ROW 15.25 COL 70 COLON-ALIGNED WIDGET-ID 8
...
/* SETTINGS FOR FILL-IN dt-max IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN dt-min IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN vl-max IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN vl-min IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
IF AVAIL moeda AND moeda.cd-moeda = 0 THEN
    DISABLE btAdd-page1 WITH FRAME {&FRAME-NAME}.
ELSE
    ENABLE btAdd-page1 WITH FRAME {&FRAME-NAME}.
...
IF NOT AVAIL {&Table} THEN DO:
    ...
    ASSIGN moeda.cd-moeda:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ?
           moeda.ds-moeda:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
           vl-max:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
           dt-max:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
           vl-max:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
           vl-max:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
ELSE DO:
    ...
    FOR EACH bf-cotacao OF moeda NO-LOCK:
        IF bf-cotacao.valor > vl-max THEN
            ASSIGN vl-max = bf-cotacao.valor
                   dt-max = bf-cotacao.dt-ini.
        IF bf-cotacao.valor < vl-min THEN
            ASSIGN vl-min = bf-cotacao.valor
                   dt-min = bf-cotacao.dt-ini.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY dt-min vl-min dt-max vl-max 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  IF AVAILABLE moeda THEN 
    DISPLAY moeda.cd-moeda moeda.ds-moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE br-page1 btAdd-page1 btDel-page1 btExit btFirst btLast btNext btPrev 
         btUpdate-page1 moeda.cd-moeda moeda.ds-moeda btGoTo btZoom RECT-5 
         RECT-6 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
