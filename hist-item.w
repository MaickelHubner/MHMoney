...
&GLOBAL-DEFINE ParentFields  item.cd-item item.ds-item item.unidade
&GLOBAL-DEFINE KeyFields     item.cd-item
&GLOBAL-DEFINE CamposFilho   compra.dt-compra ~
                             fnCd-favorecido(compra.cd-favorecido) @ cCd-favorecido ~
...
&Scoped-define INTERNAL-TABLES {&Filho} compra
...
&Scoped-define QUERY-STRING-br-page1 FOR EACH {&Filho} OF {&Table}, ~
           EACH compra OF {&Filho} NO-LOCK     BY compra.dt-compra DESCENDING. RUN controlButtons IN THIS-PROCEDURE
&Scoped-define OPEN-QUERY-br-page1 OPEN QUERY {&SELF-NAME} FOR EACH {&Filho} OF {&Table}, ~
           EACH compra OF {&Filho} NO-LOCK     BY compra.dt-compra DESCENDING. RUN controlButtons IN THIS-PROCEDURE.
&Scoped-define TABLES-IN-QUERY-br-page1 {&Filho} compra
&Scoped-define FIRST-TABLE-IN-QUERY-br-page1 {&Filho}
&Scoped-define SECOND-TABLE-IN-QUERY-br-page1 compra
...
&Scoped-Define ENABLED-FIELDS item.cd-item item.ds-item item.unidade 
...
&Scoped-Define DISPLAYED-FIELDS item.cd-item item.ds-item item.unidade 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SIZE 93 BY 10.5
         FONT 7
         TITLE "".
...
DEFINE FRAME DEFAULT-FRAME
     ...
     btAdd-page1 AT ROW 1.5 COL 46
     btDel-page1 AT ROW 1.5 COL 58.29
     ...
     btUpdate-page1 AT ROW 1.5 COL 52.14
     item.cd-item AT ROW 4.5 COL 21 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8 BY .79
     item.ds-item AT ROW 4.5 COL 29.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 43 BY .79
     btGoTo AT ROW 1.5 COL 29
     btZoom AT ROW 1.5 COL 35.14
     item.unidade AT ROW 4.5 COL 72.57 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 5 BY .79
...
/* Query rebuild information for BROWSE br-page1
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH {&Filho} OF {&Table},
    EACH compra OF {&Filho} NO-LOCK
    BY compra.dt-compra DESCENDING.
...
ASSIGN btAdd-page1:HIDDEN IN FRAME {&FRAME-NAME} = YES
       btUpdate-page1:HIDDEN IN FRAME {&FRAME-NAME} = YES
...
IF NOT AVAIL {&Table} THEN DO:
    ...
    ASSIGN item.cd-item:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ?
           item.ds-item:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
------------------------------------------------------------------------------*/
  IF AVAILABLE item THEN 
    DISPLAY item.cd-item item.ds-item item.unidade 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE br-page1 btAdd-page1 btDel-page1 btExit btFirst btLast btNext btPrev 
         btUpdate-page1 item.cd-item item.ds-item btGoTo btZoom item.unidade 
         RECT-5 RECT-6 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
