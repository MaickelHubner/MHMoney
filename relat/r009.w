...
DEFINE VARIABLE deMAnt AS DECIMAL    NO-UNDO.
DEFINE VARIABLE deMAt AS DECIMAL    NO-UNDO.
DEFINE VARIABLE deDif AS DECIMAL    NO-UNDO.
DEFINE VARIABLE dePc AS DECIMAL    NO-UNDO.
DEFINE VARIABLE cTemp AS CHARACTER  NO-UNDO FORMAT "X(60)".
DEFINE VARIABLE deTemp1 AS DECIMAL    NO-UNDO.
DEFINE VARIABLE deTemp2 AS DECIMAL    NO-UNDO.
...
END.
ELSE MESSAGE "r009.wrx":U SKIP(1)
...
ASSIGN cParamRelat = "mes-ini=" + STRING(MONTH(DATE(MONTH(DATE(mes, 1, ano) - 1), 1, YEAR(DATE(mes, 1, ano) - 1))),">9") +
                     ",ano-ini=" + STRING(YEAR(DATE(MONTH(DATE(mes, 1, ano) - 1), 1, YEAR(DATE(mes, 1, ano) - 1))),"9999") +
                     ",mes-fim=" + STRING(mes,">9") +
                     ",ano-fim=" + STRING(ano,"9999") +
                     ",moeda=" + STRING(moeda).
RUN func\gera_graf.p (INPUT "graf\g005.w", INPUT "gr001.gif", INPUT 400, INPUT 300).
ASSIGN cParamRelat = "".
PUT "  <tr>" SKIP
...
FOR EACH mov-conta NO-LOCK
    WHERE mov-conta.dt-mov >= DATE(IF mes <> 1 THEN (mes - 1) ELSE 12,1,IF mes <> 1 THEN ano ELSE (ano - 1))
    AND   mov-conta.dt-mov <= (DATE(IF mes <> 12 THEN (mes + 1) ELSE 1,1,IF mes <> 12 THEN ano ELSE (ano + 1)) - 1)
    AND   mov-conta.id-tipo <> 3
    AND   mov-conta.id-tipo <> 0
    AND   mov-conta.de-valor > 0:

    IF mov-conta.cd-moeda = moeda THEN DO:
        IF MONTH(mov-conta.dt-mov) = mes THEN
...
IF CAN-FIND(FIRST conta
    ...
    RUN var_camb.p (INPUT (DATE(mes, 1, ano) - 1), 
                    INPUT moeda,
                    OUTPUT deMAnt).

    RUN var_camb.p (INPUT DATE(mes, 1, ano), 
                    INPUT moeda,
                    OUTPUT deMAt).
...
FOR EACH mov-conta NO-LOCK
    ...
    IF MONTH(mov-conta.dt-mov) = mes THEN DO:
        ASSIGN deMAt = deMAt + fnCotacao(mov-conta.de-valor,mov-conta.cd-moeda,moeda,mov-conta.dt-mov).
        FIND FIRST tt-comp2
...
{func\relat\capitulo.i '"20 Principais Despesas Mensais"' 800}

ASSIGN cParamRelat = "data-ini=" + STRING(DATE(MONTH(DATE(mes, 1, ano) - 1), 1, YEAR(DATE(mes, 1, ano) - 1)),"99/99/9999") +
                     ",data-fim=" + STRING(fnUltimoDia(MONTH(DATE(mes, 1, ano) - 1), YEAR(DATE(mes, 1, ano) - 1)),"99/99/9999") +
                     ",tipo=2,agrupa=25" +
                     ",moeda=" + STRING(moeda).
RUN func\gera_graf.p (INPUT "graf\g016.w", INPUT "gr002.gif", INPUT 380, INPUT 300).
ASSIGN cParamRelat = "".
ASSIGN cParamRelat = "data-ini=" + STRING(DATE(mes, 1, ano),"99/99/9999") +
                     ",data-fim=" + STRING(fnUltimoDia(mes, ano),"99/99/9999") +
                     ",tipo=2,agrupa=25" +
                     ",moeda=" + STRING(moeda).
...
{func\relat\end-capitulo.i}
{func\relat\capitulo.i '' 800}
{func\relat\titulo.i &Coluna1='"Màs Anterior"'
                     &Tamanho1=270
                     &Coluna2='"Valor"'
                     &Tamanho2=80
                     &Coluna3=''
...
{func\relat\titulo.i &Coluna1='"Neste màs vocà n∆o gastou com:"'
                     &Tamanho1=400
                     &Coluna2=''
                     &Tamanho2=200}
...
{func\relat\capitulo.i '"Patrimìnio Financeiro"' 800}
...
{func\relat\control.i}
{func\relat\linha.i &Coluna1='"In°cio do Ano"'
                    &Align1="left"
                    &Coluna2=deTemp1
                    &Align2="right"}
RUN patrimon-tot.p (INPUT (DATE(mes,1,ano) - 1), INPUT moeda, OUTPUT deTemp1).
{func\relat\control.i}
{func\relat\linha.i &Coluna1='"Màs Anterior"'
                    &Align1="left"
                    &Coluna2=deTemp1
                    &Align2="right"}
RUN patrimon-tot.p (INPUT fnUltimoDia(mes, ano), INPUT moeda, OUTPUT deTemp2).
{func\relat\control.i}
{func\relat\linha.i &Coluna1='"Màs Atual"'
                  &Align1="left"
                  &Coluna2=deTemp2
                  &Align2="right"}
{func\relat\control.i}
...
{func\relat\capitulo.i '"Evoluá∆o de Saldo no Màs"' 800}
