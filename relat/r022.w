...
&GLOBAL-DEFINE FrameFields  data-ini data-fim moeda tipo cat iCd-favorecido fav
...
data-fim iCd-favorecido fav tipo cat moeda btOK 
&Scoped-Define DISPLAYED-OBJECTS data-ini data-fim iCd-favorecido fav tipo ~
cat moeda 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     iCd-favorecido AT ROW 3 COL 24 COLON-ALIGNED WIDGET-ID 2
     fav AT ROW 3 COL 29.29 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     tipo AT ROW 4 COL 24 COLON-ALIGNED
     cat AT ROW 5 COL 24 COLON-ALIGNED
     moeda AT ROW 6 COL 24 COLON-ALIGNED
...
ON ENTRY OF iCd-favorecido IN FRAME DEFAULT-FRAME /* Favorecido */
DO:
    {func\set_entry.i}  
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL iCd-favorecido C-Win
ON LEAVE OF iCd-favorecido IN FRAME DEFAULT-FRAME /* Favorecido */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL iCd-favorecido C-Win
ON MOUSE-SELECT-DBLCLICK OF iCd-favorecido IN FRAME DEFAULT-FRAME /* Favorecido */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tipo C-Win
ON VALUE-CHANGED OF tipo IN FRAME DEFAULT-FRAME /* Tipo */
DO:

    ASSIGN cat:LIST-ITEM-PAIRS = "Todas,0".
  
    FOR EACH categoria NO-LOCK
        WHERE categoria.id-tipo = INPUT FRAME {&FRAME-NAME} tipo
        BY categoria.ds-categoria:

        ASSIGN cat:LIST-ITEM-PAIRS = cat:LIST-ITEM-PAIRS + "," + categoria.ds-categoria + "," + STRING(categoria.cod-categoria).

    END.

    ASSIGN cat = 0.

    DISP cat WITH FRAME {&FRAME-NAME}.
...
END.
ELSE MESSAGE "r022.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY data-ini data-fim iCd-favorecido fav tipo cat moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btCancel IMAGE-5 IMAGE-6 RECT-10 data-ini data-fim iCd-favorecido fav 
         tipo cat moeda btOK 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
SELECT COUNT(*) INTO iMax FROM mov-conta
    ...
    AND   mov-conta.cd-favorecido = iCd-favorecido
    AND   mov-conta.id-tipo <> 3
    AND   mov-conta.id-tipo <> 0
    AND   (IF tipo = 1 THEN mov-conta.de-valor > 0 ELSE mov-conta.de-valor < 0)
    AND   (IF cat <> 0 THEN mov-conta.cod-categoria = cat ELSE TRUE).
{func\relat\ini.i iMax}

/* L¢gica do relat¢rio */

FIND FIRST favorecido NO-LOCK
    WHERE favorecido.cd-favorecido = iCd-favorecido.
ASSIGN cTemp = "De: " + STRING(data-ini,"99/99/9999") + " At‚: " + STRING(data-fim,"99/99/9999") + "<br>" +
               "Favorecido: " + STRING(iCd-favorecido, "9999") + " - " + favorecido.ds-favorecido.
...
FOR EACH mov-conta NO-LOCK
    ...
    FIND FIRST tt-comp
        WHERE tt-comp.cod-categoria = mov-conta.cod-categoria
        AND   tt-comp.cd-sub        = mov-conta.cd-sub NO-ERROR.
    ...
    ELSE DO:
        ...
        ASSIGN tt-comp.valor = fnCotacao(mov-conta.de-valor,mov-conta.cd-moeda,moeda,mov-conta.dt-mov)
               ...
               tt-comp.cd-sub = mov-conta.cd-sub.
    END.
...
FOR EACH tt-comp:
    ...
    FIND FIRST sub-cat OF categoria
        WHERE sub-cat.cd-sub = tt-comp.cd-sub.

    ASSIGN cTemp = categoria.ds-categoria + ":" + sub-cat.ds-sub.
    {func\relat\linha.i &Coluna1=cTemp
                        &Align1="left"
                        &Coluna2=tt-comp.valor
                        &Align2="right"}
                        
    ASSIGN deTot = deTot + tt-comp.valor.
...
------------------------------------------------------------------------------*/
    ...
    IF INPUT FRAME {&FRAME-NAME} iCd-favorecido = 0 THEN DO:
        {func\valida.i &Mensagem  = "Favorecido inv lido!"
                       &Ajuda     = "NÆo foi informado um favorecido v lido."
                       &Campo     = iCd-favorecido}
    END.
