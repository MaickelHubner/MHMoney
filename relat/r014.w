...
&GLOBAL-DEFINE FrameFields  mes-ini ano-ini mes-fim ano-fim moeda iItem
...
&Scoped-Define ENABLED-OBJECTS btCancel IMAGE-5 IMAGE-6 RECT-10 iItem ~
mes-ini ano-ini mes-fim ano-fim moeda btOK 
&Scoped-Define DISPLAYED-OBJECTS iItem des-item mes-ini ano-ini mes-fim ~
ano-fim moeda 
...
DEFINE VARIABLE des-item AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS FILL-IN 
     SIZE 44 BY .79 NO-UNDO.

DEFINE VARIABLE iItem AS INTEGER FORMAT "99999":U INITIAL 0 
     LABEL "Item" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     iItem AT ROW 2 COL 19 COLON-ALIGNED
     des-item AT ROW 2 COL 25.29 COLON-ALIGNED NO-LABEL
     mes-ini AT ROW 3 COL 19 COLON-ALIGNED
     ano-ini AT ROW 3 COL 35.29 COLON-ALIGNED NO-LABEL
     mes-fim AT ROW 3 COL 47.72 COLON-ALIGNED NO-LABEL
     ano-fim AT ROW 3 COL 64 COLON-ALIGNED NO-LABEL
     moeda AT ROW 4 COL 19 COLON-ALIGNED
     btOK AT ROW 2 COL 73
     IMAGE-5 AT ROW 2.58 COL 41
     IMAGE-6 AT ROW 2.58 COL 45.43
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
/* SETTINGS FOR FILL-IN des-item IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME iItem
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL iItem C-Win
ON ENTRY OF iItem IN FRAME DEFAULT-FRAME /* Item */
DO:
    {func\set_entry.i}  
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL iItem C-Win
ON LEAVE OF iItem IN FRAME DEFAULT-FRAME /* Item */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL iItem C-Win
ON MOUSE-SELECT-DBLCLICK OF iItem IN FRAME DEFAULT-FRAME /* Item */
OR F5 OF iItem DO:
...
END.
ELSE MESSAGE "r014.wrx":U SKIP(1)
...
APPLY "LEAVE" TO iItem IN FRAME {&FRAME-NAME}.
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY iItem des-item mes-ini ano-ini mes-fim ano-fim moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btCancel IMAGE-5 IMAGE-6 RECT-10 iItem mes-ini ano-ini mes-fim ano-fim 
         moeda btOK 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DEFINE VARIABLE cTemp AS CHARACTER FORMAT "X(30)" NO-UNDO.
DEFINE VARIABLE dePreco AS DECIMAL    NO-UNDO.
DEFINE VARIABLE dePreco-Ant AS DECIMAL    NO-UNDO.
DEFINE VARIABLE deTaxa AS DECIMAL FORMAT "->>>9.9999"  NO-UNDO.
DEFINE VARIABLE iCont AS INTEGER    NO-UNDO.
DEFINE VARIABLE deTotal AS DECIMAL FORMAT "->>>9.9999"    NO-UNDO.
DEFINE VARIABLE deMedio AS DECIMAL    NO-UNDO.
DEFINE VARIABLE iTotPer AS INTEGER    NO-UNDO.
DEFINE VARIABLE deIni AS DECIMAL    NO-UNDO.
DEFINE VARIABLE deMelhor AS DECIMAL    NO-UNDO.
DEFINE VARIABLE dePior AS DECIMAL    NO-UNDO.
DEFINE VARIABLE deTemp AS DECIMAL    NO-UNDO.

/* Acerto de Datas */
FIND FIRST item-compra NO-LOCK
    WHERE item-compra.cd-item = iItem NO-ERROR.
...
/* Output padr∆o */
ASSIGN iTotPer = INT((DATE(mes-fim,15,ano-fim) - DATE(mes-ini,15,ano-ini)) / 30) + 1.
{func\relat\ini.i iTotPer}

/* L¢gica do relat¢rio */

FIND FIRST item NO-LOCK
    WHERE item.cd-item = iItem NO-ERROR.

ASSIGN cTemp = "Item: " + STRING(iItem,"999") + " - " + item.ds-item + "<br>" +
               "Per°odo: " + STRING(mes-ini,"99") + "/" + STRING(ano-ini,"9999") + " atÇ " + STRING(mes-fim,"99") + "/" + STRING(ano-ini,"9999").
{func\relat\capitulo.i cTemp 400}

{func\relat\titulo.i &Coluna1='"Màs/Ano"'
                     &Tamanho1=160
                     &Coluna2='"Preáo MÇdio"'
                     &Tamanho2=120
                     &Coluna3='"Inflaá∆o"'
...
FIND FIRST item-compra OF item NO-LOCK
    WHERE CAN-FIND(FIRST compra OF item-compra
                   WHERE compra.dt-compra < DATE(mes-ini,1,ano-ini)) NO-ERROR.
IF AVAIL item-compra THEN DO:
    ASSIGN dePreco = item-compra.vl-unitario.
END.
ELSE DO:
    FIND FIRST item-compra OF item NO-LOCK
        WHERE CAN-FIND(FIRST compra OF item-compra
...
ASSIGN deIni = dePreco
       deMelhor = deIni
       dePior = deIni.

ASSIGN cTemp = fnMes(MONTH(DATE(mes-ini,1,ano-ini) - 1)) + "/" + STRING(YEAR(DATE(mes-ini,1,ano-ini) - 1),"9999").
...
REPEAT iAno = ano-ini TO ano-fim:
    REPEAT iMes = IF ano-ini = iAno THEN mes-ini ELSE 1 TO IF ano-fim = iAno THEN mes-fim ELSE 12:
        ...
        FOR EACH compra NO-LOCK
            WHERE YEAR(compra.dt-compra)  = iAno
            AND   MONTH(compra.dt-compra) = iMes,
            EACH item-compra OF compra NO-LOCK
                WHERE item-compra.cd-item = iItem:

            ASSIGN dePreco = dePreco + item-compra.vl-unitario
                   iCont = iCont + 1.

        END.

        IF dePreco = 0 THEN
            ASSIGN dePreco = dePreco-Ant.
        ELSE
            ASSIGN dePreco = (dePreco / iCont).
        
        ASSIGN deTaxa = (((dePreco - dePreco-Ant) * 100) / dePreco-Ant)
               deTotal = deTotal + deTaxa
               deMedio = deMedio + dePreco.

        IF dePreco < deMelhor THEN
            ASSIGN deMelhor = dePreco.
        IF dePreco > dePior THEN
            ASSIGN dePior = dePreco.
...
{func\relat\capitulo.i '' 400}

{func\relat\titulo.i &Coluna1='"Resumo"'
                     &Tamanho1=280
                     &Coluna2=''
                     &Tamanho2=120}
...
ASSIGN cTemp = "Preáo Inicial".
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=deIni
                    &Align2="right"}

ASSIGN cTemp = "Preáo Final".
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=dePreco
                    &Align2="right"}

ASSIGN cTemp = "Variaá∆o de Preáo"
       deTemp = (dePreco - deIni).
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=deTemp
                    &Align2="right"}

ASSIGN cTemp = "Taxa de Variaá∆o"
       deTaxa = ((deTemp * 100) / deIni).
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=deTaxa
                    &Align2="right"}

ASSIGN cTemp = "Melhor Preáo".
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=deMelhor
                    &Align2="right"}

ASSIGN cTemp = "Pior Preáo".
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=dePior
                    &Align2="right"}

ASSIGN cTemp = "Diferenáa"
       deTemp = dePior - deMelhor.
{func\relat\linha.i &Coluna1=cTemp
                    &Align1="left"
                    &Coluna2=deTemp
                    &Align2="right"}

ASSIGN cTemp = "Preáo MÇdio"
       deMedio = (deMedio / iTotPer).
