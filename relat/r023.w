...
&GLOBAL-DEFINE FrameFields  data-ini data-fim moeda tg-mes
...
data-fim tg-mes moeda btOK 
&Scoped-Define DISPLAYED-OBJECTS data-ini data-fim tg-mes moeda 
...
DEFINE VARIABLE tg-mes AS LOGICAL INITIAL no 
     LABEL "Resumo Mensal" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME DEFAULT-FRAME
     ...
     tg-mes AT ROW 3 COL 34 WIDGET-ID 2
...
END.
ELSE MESSAGE "r023.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY data-ini data-fim tg-mes moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btCancel IMAGE-5 IMAGE-6 RECT-10 data-ini data-fim tg-mes moeda btOK 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DEFINE VARIABLE deTotHrs AS DECIMAL     NO-UNDO.
DEFINE VARIABLE deTotVal AS DECIMAL     NO-UNDO.
DEFINE VARIABLE deValMed AS DECIMAL     NO-UNDO.

/* Acerto de datas */
SELECT MIN(dt-pedido) INTO data-ini FROM pedido
    WHERE pedido.dt-pedido >= data-ini.
SELECT MAX(dt-pedido) INTO data-fim FROM pedido
    WHERE pedido.dt-pedido <= data-fim.

/* Output padr∆o */
SELECT COUNT(*) INTO iMax FROM pedido
    WHERE pedido.dt-pedido >= data-ini
    AND   pedido.dt-pedido <= data-fim.
{func\relat\ini.i iMax}

/* L¢gica do relat¢rio */

ASSIGN cTemp = "De: " + STRING(data-ini,"99/99/9999") + " AtÇ: " + STRING(data-fim,"99/99/9999").
{func\relat\capitulo.i cTemp 700}

IF NOT tg-mes THEN DO:

    {func\relat\titulo.i &Coluna1='Pedido'
                         &Tamanho1=80
                         &Coluna2='Data'
                         &Tamanho2=80
                         &Coluna3='Cliente'
    ...
    FOR EACH pedido NO-LOCK
        ...
        FOR EACH apontamento OF pedido NO-LOCK:
            ...
            IF item.un MATCHES("*hr*") THEN
                ASSIGN deTotHrs = deTotHrs + apontamento.qt-apontada.
        END.

        IF deTotHrs > 0 THEN
            ASSIGN deValMed = pedido.val-total / deTotHrs.
        ELSE
            ASSIGN deValMed = pedido.val-total.

        {func\relat\linha.i &Coluna1=pedido.nr-pedido
                            &Align1="center"
                            &Coluna2=pedido.dt-pedido
                            &Align2="center"
                            &Coluna3=favorecido.ds-favorecido
                            &Align3="left"
                            &Coluna4=pedido.val-total
                            &Align4="right"
                            &Coluna5=deTotHrs
                            &Align5="right"
                            &Coluna6=deValMed
...
ELSE DO:
    ...
    FOR EACH pedido NO-LOCK
        WHERE pedido.dt-pedido >= data-ini
        AND   pedido.dt-pedido <= data-fim
        BREAK BY YEAR(pedido.dt-pedido)
        BY MONTH(pedido.dt-pedido):

        {func\relat\control.i}

        IF FIRST-OF(MONTH(pedido.dt-pedido)) THEN DO:
            ASSIGN deTotHrs = 0
                   deTotVal = 0.
        END.
        ...
        FOR EACH apontamento OF pedido NO-LOCK:
            ...
            IF item.un MATCHES("*hr*") THEN
                ASSIGN deTotHrs = deTotHrs + apontamento.qt-apontada.
        END.
        IF pedido.situacao = 3 /* Atendido */
        OR pedido.situacao = 4 /* Faturado */ THEN
            ASSIGN deTotVal = deTotVal + pedido.val-total.

        IF LAST-OF(MONTH(pedido.dt-pedido)) THEN DO:

            ASSIGN cTemp = fnMes(MONTH(pedido.dt-pedido)) + "/" + STRING(YEAR(pedido.dt-pedido)).

            IF deTotHrs > 0 THEN
                ASSIGN deValMed = deTotVal / deTotHrs.
            ELSE
                ASSIGN deValMed = deTotVal.
