...
mes-ini ano-ini mes-fim ano-fim moeda 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     mes-ini AT ROW 2 COL 17 COLON-ALIGNED WIDGET-ID 8
     ano-ini AT ROW 2 COL 33.29 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     mes-fim AT ROW 2 COL 46 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     ano-fim AT ROW 2 COL 62.29 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     moeda AT ROW 3 COL 17 COLON-ALIGNED
     IMAGE-5 AT ROW 1.63 COL 39.14
     IMAGE-6 AT ROW 1.63 COL 43.57
...
END.
ELSE MESSAGE "r030.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE btCancel IMAGE-5 IMAGE-6 RECT-10 btOK mes-ini ano-ini mes-fim ano-fim 
         moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DEFINE VARIABLE vl-ant AS DECIMAL    NO-UNDO.
DEFINE VARIABLE vl-total AS DECIMAL    NO-UNDO.
DEFINE VARIABLE pc-evol AS DECIMAL    NO-UNDO.
DEFINE VARIABLE pc-cresc AS DECIMAL    NO-UNDO.
DEFINE VARIABLE pc-ant AS DECIMAL    NO-UNDO.
DEFINE VARIABLE cMes AS CHARACTER  NO-UNDO FORMAT "X(20)".
...
{func\relat\titulo.i &Coluna1='"Mˆs"'
                     &Tamanho1=150
                     &Coluna2='"Total"'
                     &Tamanho2=150
                     &Coluna3='"Evolu‡Æo"'
...
REPEAT iAno = YEAR(data-ini) TO YEAR(data-fim):
    REPEAT iMes = IF YEAR(data-ini) <> iAno THEN 1 ELSE MONTH(data-ini) TO IF YEAR(data-fim) <> iAno THEN 12 ELSE MONTH(data-fim):
        ...
        ASSIGN cMes = fnMes(iMes) + "/" + STRING(iAno,"9999")
               pc-evol = ((vl-total - vl-ant) / vl-ant) * 100.
        IF vl-ant < 0 THEN
            ASSIGN pc-evol = (-1) * pc-evol.

        ASSIGN pc-cresc = pc-evol - pc-ant.
