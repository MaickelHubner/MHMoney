...
DEFINE FRAME DEFAULT-FRAME
     ...
     mes-ini AT ROW 2.13 COL 15 COLON-ALIGNED WIDGET-ID 8
     ano-ini AT ROW 2.13 COL 31.29 COLON-ALIGNED NO-LABEL WIDGET-ID 4
     mes-fim AT ROW 2.13 COL 44 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     ano-fim AT ROW 2.13 COL 60.29 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     moeda AT ROW 3.25 COL 15 COLON-ALIGNED
     IMAGE-5 AT ROW 1.75 COL 37.14
     IMAGE-6 AT ROW 1.75 COL 41.57
...
END.
ELSE MESSAGE "r010.wrx":U SKIP(1)
...
DEFINE VARIABLE vl-ano AS DECIMAL FORMAT "->>>>,>>>,>>9.99"   NO-UNDO.
...
DEFINE VARIABLE vl-geral AS DECIMAL FORMAT "->>>>,>>>,>>9.99" NO-UNDO.
...
REPEAT iAno = YEAR(data-ini) TO YEAR(data-fim):
    ...
    REPEAT iMes = IF YEAR(data-ini) <> iAno THEN 1 ELSE MONTH(data-ini) TO IF YEAR(data-fim) <> iAno THEN 12 ELSE MONTH(data-fim):
        ...
        RUN renddesp.p (INPUT DATE(IF iMes <> 12 THEN (iMes + 1) ELSE 1,
                                   1, IF iMes <> 12 THEN iAno ELSE (iAno + 1)) - 1,
                        INPUT moeda,
                        OUTPUT vl-total).
        ...
        {func\relat\linha.i &Coluna1=cMes
                            ...
                            &Coluna2=vl-total
                            &Align2="right"}
        ASSIGN vl-ano = vl-ano + vl-total.
    END.
    ASSIGN cMes = "TOTAL"
           vl-geral = vl-geral + vl-ano.
...
IF YEAR(data-ini) <> YEAR(data-fim) THEN DO:
    PUT "<br><br>" SKIP
        "  <tr>" SKIP
        "    <td width=~"800~" colSpan=~"2~"><strong><font face=~"Arial~" color=~"#319B64~"><br>" SKIP
        "    Total Geral: " vl-geral "<br><br></font></strong>" SKIP
        "    </td>" SKIP
        "  </tr>".
END.
