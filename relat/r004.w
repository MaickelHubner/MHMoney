...
&GLOBAL-DEFINE FrameFields  data-ini moeda
...
&Scoped-Define ENABLED-OBJECTS btCancel RECT-10 data-ini moeda btOK 
&Scoped-Define DISPLAYED-OBJECTS data-ini moeda 
...
END.
ELSE MESSAGE "r004.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY data-ini moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btCancel RECT-10 data-ini moeda btOK 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
DEFINE VARIABLE iNumContas AS INTEGER    NO-UNDO.
DEFINE VARIABLE cTemp AS CHARACTER  NO-UNDO.
DEFINE VARIABLE dePatrimonio AS DECIMAL FORMAT "->>>,>>9.99" NO-UNDO.
DEFINE VARIABLE vl-total AS DECIMAL FORMAT "->>>,>>9.99" NO-UNDO.
DEFINE VARIABLE vl-passivo AS DECIMAL FORMAT "->>>,>>9.99" NO-UNDO.

/* Output padr∆o */
SELECT COUNT(*) INTO iNumContas FROM conta.
{func\relat\ini.i iNumContas}

ASSIGN cTemp = "Data: " + STRING(data-ini,"99/99/9999").
{func\relat\capitulo.i cTemp 300}

{func\relat\titulo.i &Coluna1='Conta'
                     &Tamanho1=200
                     &Coluna2='Total'}
                         
ASSIGN dePatrimonio = 0.
FOR EACH conta NO-LOCK:

    {func\relat\control.i}
    RUN patrimonio.p (INPUT conta.cd-conta, INPUT data-ini, INPUT moeda, OUTPUT vl-total).

    /*IF NOT conta.id-encerrada THEN DO:*/
    IF vl-total <> 0 THEN DO:
        {func\relat\linha.i &Coluna1=conta.ds-conta
                            &Align1="left"
                            &Coluna2=vl-total
                            &Align2="right"}
        ASSIGN dePatrimonio = dePatrimonio + vl-total.
    END.

END.

RUN patrimon-tot.p (INPUT data-ini, INPUT moeda, OUTPUT vl-passivo).
ASSIGN vl-passivo = vl-passivo - dePatrimonio.
{func\relat\linha.i &Coluna1='"Passivo"'
                    &Align1="left"
                    &Coluna2=vl-passivo
                    &Align2="right"}
                    
ASSIGN dePatrimonio = dePatrimonio + vl-passivo.

{func\relat\total.i &Coluna1='"Patrimìnio"'
                    &Align1="left"
                    &Coluna2=dePatrimonio
                    &Align2="right"}
