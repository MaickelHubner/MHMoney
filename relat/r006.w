...
END.
ELSE MESSAGE "r006.wrx":U SKIP(1)
...
FOR EACH categoria NO-LOCK
    ...
    FOR EACH sub-cat OF categoria NO-LOCK
        WHERE sub-cat.id-tipo = 2 BY ds-sub:
...
FOR EACH categoria NO-LOCK
    ...
    FOR EACH sub-cat OF categoria NO-LOCK
        WHERE sub-cat.id-tipo = 1 BY ds-sub:
