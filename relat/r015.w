...
&GLOBAL-DEFINE FrameFields  data-ini data-fim moeda cat sub
...
data-ini data-fim cat sub moeda 
&Scoped-Define DISPLAYED-OBJECTS data-ini data-fim cat sub moeda 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     cat AT ROW 3 COL 24 COLON-ALIGNED
     sub AT ROW 4 COL 24 COLON-ALIGNED
...
DO:
    ...
    ASSIGN cList = "Todas,0".

    FOR EACH sub-cat NO-LOCK
        WHERE sub-cat.cod-categoria = INPUT FRAME {&FRAME-NAME} cat
        BY sub-cat.ds-sub:

        ASSIGN cList = cList + "," + REPLACE(sub-cat.ds-sub,","," ") + "," + STRING(sub-cat.cd-sub).

    END.

    ASSIGN sub:LIST-ITEM-PAIRS = cList.

    IF LOOKUP(STRING(sub),sub:LIST-ITEM-PAIRS) = 0 THEN
        ASSIGN sub = 0.
...
END.
ELSE MESSAGE "r015.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY data-ini data-fim cat sub moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btCancel IMAGE-5 IMAGE-6 RECT-10 btOK data-ini data-fim cat sub moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
SELECT COUNT(*) INTO iMax FROM mov-conta
    ...
    AND   (IF sub = 0 THEN TRUE ELSE mov-conta.cd-sub = sub).
...
FIND FIRST categoria NO-LOCK
    WHERE categoria.cod-categoria = cat NO-ERROR.

IF sub <> 0 THEN DO:
    FIND FIRST sub-cat OF categoria NO-LOCK
        WHERE sub-cat.cd-sub = sub NO-ERROR.
    ASSIGN cSub = sub-cat.ds-sub.
END.
ELSE DO:
    ASSIGN cSub = "Todas".
END.
...
ASSIGN cTemp = "De: " + STRING(data-ini,"99/99/9999") + " At‚: " + STRING(data-fim,"99/99/9999") + "<BR>" +
               "Categoria: " + categoria.ds-categoria + "<BR>Sub-Categoria: " + cSub.
{func\relat\capitulo.i cTemp 400}

CREATE tt-resp.
ASSIGN tt-resp.resp = "Todos".

FOR EACH usuario:
    CREATE tt-resp.
    ASSIGN tt-resp.resp = usuario.nome.
END.
...
FOR EACH mov-conta NO-LOCK
    ...
    FIND FIRST tt-resp
        WHERE tt-resp.resp = (IF mov-conta.usuar-resp <> "" THEN mov-conta.usuar-resp ELSE "Todos") NO-ERROR.
    ASSIGN tt-resp.valor = tt-resp.valor + fnCotacao(mov-conta.de-valor,mov-conta.cd-moeda,moeda,mov-conta.dt-mov).
...
FOR EACH tt-resp:
    ...
    {func\relat\linha.i &Coluna1=tt-resp.resp
                        &Align1="left"
                        &Coluna2=tt-resp.valor
                        &Align2="right"}
                        
    ASSIGN deTot = deTot + tt-resp.valor.
...
FOR EACH categoria NO-LOCK
    ...
    IF cList = ",0" THEN
        ASSIGN cList = categoria.ds-categoria + "," + STRING(categoria.cod-categoria).
    ELSE
        ASSIGN cList = cList + "," + categoria.ds-categoria + "," + STRING(categoria.cod-categoria).

END.

ASSIGN cat = INT(ENTRY(2,cList))
       cat:LIST-ITEM-PAIRS = cList.
