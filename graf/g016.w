...
&GLOBAL-DEFINE FrameFields  data-ini data-fim moeda tipo agrupa cat
...
btOK data-fim tipo cat moeda agrupa 
&Scoped-Define DISPLAYED-OBJECTS data-ini data-fim tipo cat moeda agrupa 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     cat AT ROW 4 COL 24 COLON-ALIGNED
     moeda AT ROW 5 COL 24 COLON-ALIGNED
     agrupa AT ROW 6 COL 24 COLON-ALIGNED
     "%" VIEW-AS TEXT
          SIZE 2 BY .54 AT ROW 6.13 COL 35
...
END.
ELSE MESSAGE "g016.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY data-ini data-fim tipo cat moeda agrupa 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE btCancel IMAGE-5 IMAGE-6 RECT-10 data-ini btOK data-fim tipo cat moeda 
         agrupa 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
ASSIGN tt-graf-par.dado-label     = "Sub-Categoria"
       tt-graf-par.valor-label[1] = "Valor"
       tt-graf-par.tipo = 19.

IF cat = 0 THEN
    ASSIGN tt-graf-par.titulo = tt-graf-par.titulo + "-Categorias de " + IF tipo = 1 THEN "Receita" ELSE "Despesa".
ELSE DO:
    FIND FIRST categoria NO-LOCK
        WHERE categoria.cod-categoria = cat.
    ASSIGN tt-graf-par.titulo = tt-graf-par.titulo + "-Categorias de " + categoria.ds-categoria.
END.
...
FOR EACH tt-comp:
    ...
    IF deAgrup > agrupa THEN DO:
        ...
        FIND FIRST sub-cat
            WHERE sub-cat.cod-categoria = tt-comp.cod-categoria
            AND   sub-cat.cd-sub = tt-comp.cd-sub.

        CREATE tt-graf.
        ASSIGN tt-graf.seq  = iTemp
               iTemp = iTemp + 1
               tt-graf.valor[1] = tt-comp.valor.

        IF cat = 0 THEN
            ASSIGN tt-graf.dado = categoria.ds-categoria + ":" + sub-cat.ds-sub.
        ELSE
            ASSIGN tt-graf.dado = sub-cat.ds-sub.
