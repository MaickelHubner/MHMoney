...
END.
ELSE MESSAGE "g021.wrx":U SKIP(1)
...
ASSIGN tt-graf-par.dado-label     = "Mˆs/Ano"
       tt-graf-par.valor-label[1] = "Bens"
       tt-graf-par.valor-label[2] = "Dinheiro"
       tt-graf-par.valor-label[3] = "Patrim“nio".

IF iTemp <= 13 THEN
    ASSIGN tt-graf-par.mostra-valores[3] = YES.
ELSE
    ASSIGN tt-graf-par.mostra-valores[3] = NO.
...
REPEAT iAno = YEAR(data-ini) TO YEAR(data-fim):
    ...
    REPEAT iMes = (IF YEAR(data-ini) = iAno THEN MONTH(data-ini) ELSE 1) TO (IF YEAR(data-fim) = iAno THEN MONTH(data-fim) ELSE 12):
        ...
        ASSIGN deTotal = 0
               dtTemp = (DATE(IF iMes <> 12 THEN iMes + 1 ELSE 1,1,IF iMes <> 12 THEN iAno ELSE iAno + 1) - 1).
        FOR EACH bens
            WHERE (bens.dt-venda > dtTemp OR bens.dt-venda = ?)
            AND   bens.bem-pai = 0:
            RUN valorbem.p(INPUT bens.cd-bens,INPUT dtTemp, INPUT moeda, OUTPUT deTemp).
            ASSIGN deTotal = deTotal + deTemp.
        END.
        RUN patrimon-tot.p (INPUT dtTemp, INPUT moeda, OUTPUT tt-graf.valor[2]).
        ASSIGN tt-graf.valor[1] = deTotal
               tt-graf.valor[3] = tt-graf.valor[1] + tt-graf.valor[2].
