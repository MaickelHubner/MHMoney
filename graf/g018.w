...
END.
ELSE MESSAGE "g018.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  ...
  ENABLE btCancel IMAGE-5 btOK IMAGE-6 RECT-10 mes-ini ano-ini mes-fim ano-fim 
         moeda 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
ASSIGN tt-graf-par.dado-label     = "Mˆs/Ano"
       tt-graf-par.valor-label[2] = "Despesa"
       tt-graf-par.valor-label[3] = "Custo"
       tt-graf-par.valor-label[1] = "Total".

IF iTemp <= 13 THEN
    ASSIGN tt-graf-par.mostra-valores[1] = YES
           tt-graf-par.mostra-valores[2] = YES
           tt-graf-par.mostra-valores[3] = YES.
ELSE
...
REPEAT iAno = YEAR(data-ini) TO YEAR(data-fim):
    ...
    REPEAT iMes = (IF YEAR(data-ini) = iAno THEN MONTH(data-ini) ELSE 1) TO (IF YEAR(data-fim) = iAno THEN MONTH(data-fim) ELSE 12):
        ...
        RUN custos.p ( INPUT DATE(iMes,1,iAno),
                       INPUT DATE(IF iMes <> 12 THEN (iMes + 1) ELSE 1,
                                          1, IF iMes <> 12 THEN iAno ELSE (iAno + 1)) - 1,
                       INPUT moeda,
                       OUTPUT tt-graf.valor[3]).
        RUN recXdesp.p (INPUT DATE(iMes,10,iAno), INPUT moeda, OUTPUT deAux, OUTPUT tt-graf.valor[1]).
        ASSIGN tt-graf.valor[2] = tt-graf.valor[1] - tt-graf.valor[3]
               tt-graf.valor[1] = (-1) * tt-graf.valor[1]
               tt-graf.valor[2] = (-1) * tt-graf.valor[2]
               tt-graf.valor[3] = (-1) * tt-graf.valor[3].
