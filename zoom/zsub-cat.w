...
&GLOBAL-DEFINE CamposBrowse  cd-sub ~
                             ds-sub ~
                             fnId-tipo(sub-cat.id-tipo) @ cId-tipo ~
                             l-ir ~
                             reembolso
&GLOBAL-DEFINE CamposFiltro  cod-categoria ~
                             cd-sub ~
                             ds-sub ~
                             id-tipo
...
DEFINE NEW GLOBAL SHARED VARIABLE r-categoria AS ROWID      NO-UNDO.
...
DO: 
    ...
    REPEAT iCont = 1 TO NUM-ENTRIES(gcFiltro):
        IF ENTRY(iCont,gcFiltro) MATCHES("*cod-categoria*") THEN DO:
            ASSIGN cValor = ENTRY(iCont + 1,gcFiltro).
            LEAVE.
        END.
    END.

    FIND FIRST categoria NO-LOCK
        WHERE categoria.cod-categoria = INT(cValor).
    ASSIGN r-categoria = ROWID(categoria).

    {func\zoom\bt_implantar.i &Programa="subcat"}
END.
