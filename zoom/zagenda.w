...
&GLOBAL-DEFINE CamposBrowse  cd-agenda ~
                             fnId-tipo(agenda.cd-conta,agenda.id-tipo) @ cId-tipo ~
                             fnCd-conta(agenda.cd-conta) @ cCd-conta ~
                             fnCd-Favorecido(agenda.cd-favorecido) @ cCd-favorecido ~
                             fnCod-Categoria(agenda.cod-categoria) @ cCod-categoria ~
                             fnCd-sub(agenda.cd-sub,agenda.cod-categoria) @ cCd-sub ~
                             dt-ini ~
                             dt-fim ~
                             dt-ultimo-pag ~
                             fnCd-conta(agenda.conta-transf) @ cConta-transf
&GLOBAL-DEFINE CamposFiltro  cd-agenda ~
                             id-tipo ~
                             cd-conta ~
                             cd-favorecido ~
                             cod-categoria ~
...
DEFINE VARIABLE cId-tipo AS CHARACTER FORMAT "X(18)" COLUMN-LABEL "Tipo" NO-UNDO.
DEFINE VARIABLE cConta-transf AS CHARACTER FORMAT "X(40)" COLUMN-LABEL "Conta Transferància" NO-UNDO.
...
FUNCTION fnId-tipo RETURNS CHARACTER
  ( iConta AS INT, iTipo AS INT )  FORWARD.
...
DO: 
    {func\zoom\bt_implantar.i &Programa="agend"}
END.
        ...
        CASE conta.id-tipo:
            WHEN 1 THEN DO:
                CASE iTipo:
                    ...
                    WHEN 3 THEN ASSIGN cReturn = "Transferància".
                    WHEN 4 THEN ASSIGN cReturn = "Cheque".
                    WHEN 5 THEN ASSIGN cReturn = "Pagamento".
                    WHEN 6 THEN ASSIGN cReturn = "Saque em Dinheiro".
                END CASE.
            END.
            WHEN 2 THEN DO:
                CASE iTipo:
            ...
            WHEN 3 THEN DO:
                IF NOT conta.dinheiro THEN DO:
                    CASE iTipo:
                        WHEN 2 THEN ASSIGN cReturn = "CrÇdito".
                        WHEN 3 THEN ASSIGN cReturn = "Transferància".
                        WHEN 7 THEN ASSIGN cReturn = "DÇbito".
                    END CASE.
                END.
                ELSE DO:
                    CASE iTipo:
