...
&GLOBAL-DEFINE CamposBrowse  ordem-serv.num-os ~
                             ordem-serv.ds-os ~
                             fnCd-favorecido(ordem-serv.cd-favorecido) @ cCd-favorecido ~
                             fnDtPrevTermino(ordem-serv.num-os) @ cDt-PrevTerm ~
                             ordem-serv.nome ~
                             ordem-serv.dt-os ~
                             fnSituacaoOS(ordem-serv.situacao) @ cSituacao ~
                             fnHorasRestantes(ordem-serv.num-os) @ cHr-Rest ~
                             ordem-serv.nr-pedido
                             
&GLOBAL-DEFINE CamposFiltro  num-os ~
                             nome ~
                             cd-favorecido ~
                             dt-os ~
                             situacao ~
...
{campos\situacao.i}
DEFINE VARIABLE cHr-Rest     AS DECIMAL FORMAT ">>>>9.99" COLUMN-LABEL "Hr Rest."  NO-UNDO.
DEFINE VARIABLE cDt-PrevTerm AS DATE FORMAT "99/99/9999" COLUMN-LABEL "Prev.Term." NO-UNDO.
...
&Scoped-define QUERY-STRING-brZoom FOR EACH {&Table} NO-LOCK INDEXED-REPOSITION
...
&Scoped-Define ENABLED-OBJECTS brZoom btImp btFiltro btOK btCancel 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnDtPrevTermino C-Win 
FUNCTION fnDtPrevTermino RETURNS DATE
  (INPUT p-num-os AS INTEGER)  FORWARD.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fnHorasRestantes C-Win 
FUNCTION fnHorasRestantes RETURNS DECIMAL
  (INPUT p-num-os AS INTEGER)  FORWARD.
...
ON END-ERROR OF C-Win /* Zoom de contatos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Zoom de contatos */
DO:
...
DO: 
    /*{func\zoom\bt_implantar.i &Programa="contato"}*/
END.
...
------------------------------------------------------------------------------*/
  ENABLE brZoom btImp btFiltro btOK btCancel 
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN btImp:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnDtPrevTermino C-Win 
FUNCTION fnDtPrevTermino RETURNS DATE
  (INPUT p-num-os AS INTEGER) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
    DEFINE VARIABLE dt-aux AS DATE  INITIAL 01/01/0001      NO-UNDO.

    FOR EACH tarefa NO-LOCK
        WHERE tarefa.num-os = p-num-os:
        IF tarefa.data-prev-fim > dt-aux THEN
            ASSIGN dt-aux = tarefa.data-prev-fim.
    END.

    RETURN dt-aux.   /* Function return value. */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fnHorasRestantes C-Win 
FUNCTION fnHorasRestantes RETURNS DECIMAL
  (INPUT p-num-os AS INTEGER) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
    DEFINE VARIABLE hr-tot  AS DECIMAL     NO-UNDO.
    DEFINE VARIABLE hr-real AS DECIMAL     NO-UNDO.
    ...
    IF (hr-tot - hr-real) > 0 THEN
        RETURN (hr-tot - hr-real).   /* Function return value. */
    ELSE
        RETURN 0.
