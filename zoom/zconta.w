...
&GLOBAL-DEFINE CamposBrowse  cd-conta ~
                             ds-conta ~
                             fnId-tipo(id-tipo) @ cId-tipo ~
                             dinheiro ~
                             id-encerrada
&GLOBAL-DEFINE CamposFiltro  cd-conta ~
                             ds-conta

{func\zoom\definitions.i}

DEFINE VARIABLE cId-tipo AS CHARACTER COLUMN-LABEL "Tipo" FORMAT "X(17)" NO-UNDO.
...
ON END-ERROR OF C-Win /* Zoom de contas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL C-Win C-Win
ON WINDOW-CLOSE OF C-Win /* Zoom de contas */
DO:
...
&Scoped-define BROWSE-NAME brZoom
&Scoped-define SELF-NAME brZoom
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL brZoom C-Win
ON ROW-DISPLAY OF brZoom IN FRAME DEFAULT-FRAME
DO:
  
    ASSIGN conta.id-encerrada:FORMAT IN BROWSE brZoom = "Sim/NÆo".
...
DO: 
    {func\zoom\bt_implantar.i &Programa="contas"}
END.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN whColuna = BROWSE brZoom:GET-BROWSE-COLUMN(4)
         whColuna:LABEL = "Dinheiro ?"
         whColuna:WIDTH = 6.8
         whColuna = BROWSE brZoom:GET-BROWSE-COLUMN(5)
         whColuna:LABEL = "Encerrada ?"
         whColuna:WIDTH = 8.5.
...
FUNCTION fnId-tipo RETURNS CHARACTER
    ...
    CASE iCod:
        WHEN 1 THEN RETURN "Conta Corrente".
        WHEN 2 THEN RETURN "Poupan‡a".
        WHEN 3 THEN RETURN "CartÆo de Cr‚dito".
    END CASE.
