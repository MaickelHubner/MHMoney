...
&Scoped-Define ENABLED-OBJECTS btPrint RECT-11 btSave btExit 
...
DEFINE BUTTON btPrint 
     ...
     LABEL "btexit 2" 
     SIZE 6.14 BY 1.79 TOOLTIP "Imprimir".
...
ON CHOOSE OF btPrint IN FRAME DEFAULT-FRAME /* btexit 2 */
DO:
  
    RUN som.p(INPUT "music\click.wav").

    DEFINE VARIABLE IDM_PRINT AS INTEGER NO-UNDO INITIAL 6.
    DEFINE VARIABLE OLECMDEXECOPT_DONTPROMPTUSER AS INTEGER NO-UNDO INITIAL 2.

    ChCtrlFrame:WebBrowser:ExecWB( IDM_PRINT, 
...
DO:
    ...
    IF OKpressed  THEN  DO:
        ...
        OS-COPY VALUE(cArquivo) VALUE(procname).
...
END.
ELSE MESSAGE "relview.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  RUN control_load.
  ENABLE btPrint RECT-11 btSave btExit 
...
------------------------------------------------------------------------------*/
  ...
  chCtrlFrame:WebBrowser:Navigate2(FILE-INFO:FULL-PATHNAME).
