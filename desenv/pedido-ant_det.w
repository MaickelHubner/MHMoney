...
&GLOBAL-DEFINE FrameFields  pedido.dt-pedido pedido.val-total ~
                            pedido.dt-previsao-fat pedido.nom-projeto pedido.situacao
&GLOBAL-DEFINE FrameDisp    nom-projeto-pai
...
pedido.nr-pedido pedido.nom-projeto pedido.dt-pedido pedido.val-total ~
pedido.dt-previsao-fat pedido.situacao 
...
pedido.nom-projeto pedido.dt-pedido pedido.val-total pedido.dt-previsao-fat ~
pedido.situacao 
&Scoped-define ENABLED-TABLES pedido
&Scoped-define FIRST-ENABLED-TABLE pedido
&Scoped-Define ENABLED-OBJECTS nom-projeto-pai RECT-2 RECT-7 
...
&Scoped-Define DISPLAYED-OBJECTS nom-projeto-pai 
...
DEFINE VARIABLE nom-projeto-pai AS CHARACTER FORMAT "X(80)":U 
     VIEW-AS FILL-IN 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     nom-projeto-pai AT ROW 1.5 COL 24 COLON-ALIGNED NO-LABEL WIDGET-ID 62
     pedido.nr-pedido AT ROW 2.5 COL 16 COLON-ALIGNED WIDGET-ID 50
          VIEW-AS FILL-IN 
          SIZE 8 BY .79
     pedido.nom-projeto AT ROW 2.5 COL 24 COLON-ALIGNED NO-LABEL WIDGET-ID 52
          VIEW-AS FILL-IN 
          SIZE 52 BY .79
     btOK AT ROW 6.21 COL 67.86
     pedido.dt-pedido AT ROW 4 COL 16 COLON-ALIGNED WIDGET-ID 44
          LABEL "Data Antencipaá∆o"
          VIEW-AS FILL-IN 
          SIZE 11 BY .79
     pedido.val-total AT ROW 4 COL 45 COLON-ALIGNED WIDGET-ID 46
          LABEL "Valor Antecipaá∆o"
          VIEW-AS FILL-IN 
          SIZE 17 BY .79
     pedido.dt-previsao-fat AT ROW 5 COL 16 COLON-ALIGNED WIDGET-ID 60
          VIEW-AS FILL-IN 
          SIZE 11 BY .79
     pedido.situacao AT ROW 5 COL 45 COLON-ALIGNED WIDGET-ID 26
          VIEW-AS COMBO-BOX INNER-LINES 7
          LIST-ITEM-PAIRS "Pendente",3,
                     "Faturado",4,
                     "Cancelado",9
          DROP-DOWN-LIST
          SIZE 17 BY .88
...
/* SETTINGS FOR BUTTON btOK IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN pedido.dt-pedido IN FRAME DEFAULT-FRAME
   EXP-LABEL                                                            */
/* SETTINGS FOR FILL-IN pedido.val-total IN FRAME DEFAULT-FRAME
   EXP-LABEL                                                            */
...
ON ENTRY OF pedido.dt-pedido IN FRAME DEFAULT-FRAME /* Data Antencipaá∆o */
DO:
    {func\set_entry.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL pedido.dt-pedido C-Win
ON LEAVE OF pedido.dt-pedido IN FRAME DEFAULT-FRAME /* Data Antencipaá∆o */
DO:
    {func\set_leave.i}
END.
...
&Scoped-define SELF-NAME pedido.nom-projeto
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL pedido.nom-projeto C-Win
ON ENTRY OF pedido.nom-projeto IN FRAME DEFAULT-FRAME /* Projeto */
DO:
    {func\set_entry.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL pedido.nom-projeto C-Win
ON LEAVE OF pedido.nom-projeto IN FRAME DEFAULT-FRAME /* Projeto */
DO:
    {func\set_leave.i}
END.
...
DO:
    ...
    IF AVAIL bf-pedido THEN
        ASSIGN nom-projeto-pai:SCREEN-VALUE IN FRAME {&FRAME-NAME}   = bf-pedido.nom-projeto.
    ELSE
        ASSIGN nom-projeto-pai:SCREEN-VALUE IN FRAME {&FRAME-NAME}   = "".
...
&Scoped-define SELF-NAME pedido.val-total
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL pedido.val-total C-Win
ON ENTRY OF pedido.val-total IN FRAME DEFAULT-FRAME /* Valor Antecipaá∆o */
DO:
    {func\set_entry.i}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL pedido.val-total C-Win
ON LEAVE OF pedido.val-total IN FRAME DEFAULT-FRAME /* Valor Antecipaá∆o */
DO:
    {func\set_leave.i}
END.
...
------------------------------------------------------------------------------*/
    ...
    BUFFER-COPY bf-ped-pai EXCEPT nr-controle situacao nom-projeto nr-pedido dt-pedido val-total dt-previsao-fat nr-pedido-ant TO pedido.
...
IF INPUT FRAME DEFAULT-FRAME pedido.nom-projeto = "" THEN
    ASSIGN pedido.nom-projeto:SCREEN-VALUE IN FRAME DEFAULT-FRAME = nom-projeto-pai:SCREEN-VALUE IN FRAME DEFAULT-FRAME.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY nom-projeto-pai 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  IF AVAILABLE pedido THEN 
    DISPLAY pedido.nr-pedido-ant pedido.nr-pedido pedido.nom-projeto 
          pedido.dt-pedido pedido.val-total pedido.dt-previsao-fat 
          pedido.situacao 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE pedido.nr-pedido-ant nom-projeto-pai pedido.nr-pedido 
         pedido.nom-projeto pedido.dt-pedido pedido.val-total 
         pedido.dt-previsao-fat pedido.situacao RECT-2 RECT-7 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
