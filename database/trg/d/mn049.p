TRIGGER PROCEDURE FOR DELETE OF pedido-item-usu.
...
IF param-mn.sincroniza THEN DO:
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.antes   = TRIM(STRING(pedido-item-usu.nr-pedido,">>>>,>>9")).
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.chave   = yes
           lote-mov-campo.origem  = param-mn.maquina-local
           lote-mov-campo.antes   = TRIM(STRING(pedido-item-usu.sequencia,">>>9")).
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.chave   = yes
           lote-mov-campo.origem  = param-mn.maquina-local
           lote-mov-campo.antes   = TRIM(STRING(pedido-item-usu.nome,"X(8)")).
