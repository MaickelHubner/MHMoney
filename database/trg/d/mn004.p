TRIGGER PROCEDURE FOR DELETE OF mov-conta.
...
IF param-mn.sincroniza THEN DO:
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.antes   = TRIM(STRING(mov-conta.cd-conta,"999")).
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.chave   = yes
           lote-mov-campo.origem  = param-mn.maquina-local
           lote-mov-campo.antes   = TRIM(STRING(mov-conta.cd-sequencia,"->,>>>,>>9")).
