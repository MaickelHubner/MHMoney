TRIGGER PROCEDURE FOR DELETE OF agenda-valor.
...
IF param-mn.sincroniza THEN DO:
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.antes   = TRIM(STRING(agenda-valor.cd-agenda,"999")).
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.chave   = yes
           lote-mov-campo.origem  = param-mn.maquina-local
           lote-mov-campo.antes   = TRIM(STRING(agenda-valor.dt-ini,"99/99/9999")).
