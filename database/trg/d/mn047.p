TRIGGER PROCEDURE FOR DELETE OF nota-fiscal-imp.
...
IF param-mn.sincroniza THEN DO:
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.antes   = TRIM(STRING(nota-fiscal-imp.nr-controle,">>>>>>9")).
    ...
    ASSIGN lote-mov-campo.num-mov = iNum
           ...
           lote-mov-campo.chave   = yes
           lote-mov-campo.origem  = param-mn.maquina-local
           lote-mov-campo.antes   = TRIM(STRING(nota-fiscal-imp.cod-imposto,"999")).
