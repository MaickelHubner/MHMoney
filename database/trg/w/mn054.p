TRIGGER PROCEDURE FOR WRITE OF custo NEW BUFFER newTab OLD BUFFER oldTab.
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "periodo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.periodo,"x(6)")).
                END.
                WHEN "qt-horas" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.qt-horas,"->>,>>9.99")).
                END.
                WHEN "val-custo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.val-custo,"->>,>>9.99")).
                END.
                WHEN "valor" THEN DO:
        ...
        WHEN "OLD" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "periodo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.periodo,"x(6)")).
                END.
                WHEN "qt-horas" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.qt-horas,"->>,>>9.99")).
                END.
                WHEN "val-custo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.val-custo,"->>,>>9.99")).
                END.
                WHEN "valor" THEN DO:
...
IF param-mn.sincroniza THEN DO:
    ...
    OR LOOKUP("periodo",cResult) <> 0 THEN DO:
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(newTab.periodo,"x(6)")).
    ...
    ELSE DO:
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(oldTab.periodo,"x(6)")).
               lote-mov-campo.depois  = TRIM(STRING(newTab.periodo,"x(6)")).
