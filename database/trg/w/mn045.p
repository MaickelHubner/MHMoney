TRIGGER PROCEDURE FOR WRITE OF parc-nota-fiscal NEW BUFFER newTab OLD BUFFER oldTab.
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "dt-pagto-parc" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.dt-pagto-parc,"99/99/9999")).
                END.
                WHEN "dt-parcela" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.dt-parcela,"99/99/9999")).
                END.
                ...
                END.
                WHEN "val-parcela" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.val-parcela,"->>,>>9.99")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "dt-pagto-parc" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.dt-pagto-parc,"99/99/9999")).
                END.
                WHEN "dt-parcela" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.dt-parcela,"99/99/9999")).
                END.
                WHEN "nr-controle" THEN DO:
                ...
                END.
                WHEN "val-parcela" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.val-parcela,"->>,>>9.99")).
                END.
            END CASE.
        END.
    END CASE.
...
IF param-mn.sincroniza THEN DO:
    ...
    ASSIGN lote-mov.usuario  = gcUsuario
           ...
           lote-mov.tabela   = "parc-nota-fiscal"
