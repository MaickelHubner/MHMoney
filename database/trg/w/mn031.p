TRIGGER PROCEDURE FOR WRITE OF compra NEW BUFFER newTab OLD BUFFER oldTab.
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "dt-compra" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.dt-compra,"99/99/9999")).
                END.
                WHEN "gerada" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.gerada,"Sim/NÆo")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "dt-compra" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.dt-compra,"99/99/9999")).
                END.
                WHEN "gerada" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.gerada,"Sim/NÆo")).
                END.
            END CASE.
        END.
    END CASE.
...
IF param-mn.sincroniza THEN DO:
    ...
    IF LOOKUP("cd-compra",cResult) <> 0 THEN DO:
