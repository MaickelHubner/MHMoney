TRIGGER PROCEDURE FOR WRITE OF cotacao NEW BUFFER newTab OLD BUFFER oldTab.
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "valor" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.valor,"->>,>>9.999999")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "valor" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.valor,"->>,>>9.999999")).
                END.
            END CASE.
        END.
    END CASE.
...
IF param-mn.sincroniza THEN DO:
    ...
    IF LOOKUP("cd-moeda",cResult) <> 0
    OR LOOKUP("dt-ini",cResult) <> 0 THEN DO:

        ASSIGN lote-mov.operacao = "CRE".

        CREATE lote-mov-campo.
        ASSIGN lote-mov-campo.num-mov = iNum
               lote-mov-campo.campo   = "cd-moeda"
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(newTab.cd-moeda,">>9")).

        CREATE lote-mov-campo.
        ASSIGN lote-mov-campo.num-mov = iNum
               lote-mov-campo.campo   = "dt-ini"
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(newTab.dt-ini,"99/99/9999")).
    ...
    ELSE DO:
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(oldTab.cd-moeda,">>9")).
               lote-mov-campo.depois  = TRIM(STRING(newTab.cd-moeda,">>9")).
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(oldTab.dt-ini,"99/99/9999")).
               lote-mov-campo.depois  = TRIM(STRING(newTab.dt-ini,"99/99/9999")).
