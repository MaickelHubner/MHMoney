TRIGGER PROCEDURE FOR WRITE OF ped-exec NEW BUFFER newTab OLD BUFFER oldTab.
...
FUNCTION fnValor RETURNS CHARACTER (cTab AS CHARACTER, cCampo AS CHARACTER):
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                WHEN "ds-caminho" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-caminho,"x(200)")).
                END.
                WHEN "ds-pedido-exec" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-pedido-exec,"x(40)")).
                END.
                WHEN "dt-pedido-exec" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.dt-pedido-exec,"99/99/9999")).
                END.
                WHEN "hr-pedido-exec" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.hr-pedido-exec,"x(8)")).
                END.
                WHEN "nome" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.nome,"X(8)")).
                END.
                WHEN "num-periodo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.num-periodo,">>9")).
                END.
                WHEN "sequencia" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.sequencia,">>>>>>9")).
                END.
                WHEN "tipo-periodo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.tipo-periodo,"9")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                WHEN "ds-caminho" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-caminho,"x(200)")).
                END.
                WHEN "ds-pedido-exec" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-pedido-exec,"x(40)")).
                END.
                WHEN "dt-pedido-exec" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.dt-pedido-exec,"99/99/9999")).
                END.
                WHEN "hr-pedido-exec" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.hr-pedido-exec,"x(8)")).
                END.
                WHEN "nome" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.nome,"X(8)")).
                END.
                WHEN "num-periodo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.num-periodo,">>9")).
                END.
                WHEN "sequencia" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.sequencia,">>>>>>9")).
                END.
                WHEN "tipo-periodo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.tipo-periodo,"9")).
                END.
            END CASE.
        END.
    END CASE.
