TRIGGER PROCEDURE FOR WRITE OF usuario NEW BUFFER newTab OLD BUFFER oldTab.
...
FUNCTION fnValor RETURNS CHARACTER (cTab AS CHARACTER, cCampo AS CHARACTER):
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                WHEN "administrador" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.administrador,"Sim/NÆo")).
                END.
                WHEN "aponta-sem-tarefa" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.aponta-sem-tarefa,"yes/no")).
                END.
                WHEN "cd-favorecido" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.cd-favorecido,"9999")).
                END.
                WHEN "cor-campo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.cor-campo,">>9")).
                END.
                WHEN "hrs-dia" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.hrs-dia,"->>,>>9.99")).
                END.
                WHEN "nome" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.nome,"X(8)")).
                END.
                WHEN "performance" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.performance,">>9.9999")).
                END.
                WHEN "senha" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.senha,"X(8)")).
                END.
                WHEN "skin" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.skin,"X(40)")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                WHEN "administrador" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.administrador,"Sim/NÆo")).
                END.
                WHEN "aponta-sem-tarefa" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.aponta-sem-tarefa,"yes/no")).
                END.
                WHEN "cd-favorecido" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.cd-favorecido,"9999")).
                END.
                WHEN "cor-campo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.cor-campo,">>9")).
                END.
                WHEN "hrs-dia" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.hrs-dia,"->>,>>9.99")).
                END.
                WHEN "nome" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.nome,"X(8)")).
                END.
                WHEN "performance" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.performance,">>9.9999")).
                END.
                WHEN "senha" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.senha,"X(8)")).
                END.
                WHEN "skin" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.skin,"X(40)")).
                END.
            END CASE.
        END.
    END CASE.
...
IF param-mn.sincroniza THEN DO:
    ...
    IF LOOKUP("nome",cResult) <> 0 THEN DO:
