TRIGGER PROCEDURE FOR WRITE OF forma-pagto NEW BUFFER newTab OLD BUFFER oldTab.
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "cd-forma-pagto" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.cd-forma-pagto,"9999")).
                END.
                WHEN "ds-forma-pagto" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-forma-pagto,"X(40)")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                ...
                END.
                WHEN "cd-forma-pagto" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.cd-forma-pagto,"9999")).
                END.
                WHEN "ds-forma-pagto" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-forma-pagto,"X(40)")).
                END.
            END CASE.
        END.
    END CASE.
...
IF param-mn.sincroniza THEN DO:
    ...
    ELSE DO:
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(oldTab.cd-forma-pagto,"9999")).
               lote-mov-campo.depois  = TRIM(STRING(newTab.cd-forma-pagto,"9999")).
