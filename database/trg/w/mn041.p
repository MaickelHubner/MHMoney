TRIGGER PROCEDURE FOR WRITE OF pedido-item-anexo NEW BUFFER newTab OLD BUFFER oldTab.
...
FUNCTION fnValor RETURNS CHARACTER (cTab AS CHARACTER, cCampo AS CHARACTER):
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                WHEN "ds-arquivo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-arquivo,"x(80)")).
                END.
                WHEN "ds-observacao" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-observacao,"x(2000)")).
                END.
                WHEN "ds-sub-pasta" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-sub-pasta,"x(20)")).
                END.
                WHEN "nr-anexo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.nr-anexo,">>9")).
                END.
        ...
        WHEN "OLD" THEN DO:
            CASE cCampo:
                WHEN "ds-arquivo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-arquivo,"x(80)")).
                END.
                WHEN "ds-observacao" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-observacao,"x(2000)")).
                END.
                WHEN "ds-sub-pasta" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-sub-pasta,"x(20)")).
                END.
                WHEN "nr-anexo" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.nr-anexo,">>9")).
                END.
                WHEN "nr-pedido" THEN DO:
...
IF param-mn.sincroniza THEN DO:
    ...
    ASSIGN lote-mov.usuario  = gcUsuario
           ...
           lote-mov.tabela   = "pedido-item-anexo"
    ...
    OR LOOKUP("nr-anexo",cResult) <> 0 THEN DO:
        ...
        CREATE lote-mov-campo.
        ASSIGN lote-mov-campo.num-mov = iNum
               lote-mov-campo.campo   = "nr-anexo"
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(newTab.nr-anexo,">>9")).
    ...
    ELSE DO:
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(oldTab.nr-anexo,">>9")).
               lote-mov-campo.depois  = TRIM(STRING(newTab.nr-anexo,">>9")).
