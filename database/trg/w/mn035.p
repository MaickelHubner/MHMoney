TRIGGER PROCEDURE FOR WRITE OF feriado NEW BUFFER newTab OLD BUFFER oldTab.
...
FUNCTION fnValor RETURNS CHARACTER (cTab AS CHARACTER, cCampo AS CHARACTER):
    ...
    CASE cTab:
        WHEN "NEW" THEN DO:
            CASE cCampo:
                WHEN "ds-feriado" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.ds-feriado,"X(80)")).
                END.
                WHEN "dt-feriado" THEN DO:
                    ASSIGN cValor = TRIM(STRING(newTab.dt-feriado,"99/99/9999")).
                END.
            END CASE.
        END.
        WHEN "OLD" THEN DO:
            CASE cCampo:
                WHEN "ds-feriado" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.ds-feriado,"X(80)")).
                END.
                WHEN "dt-feriado" THEN DO:
                    ASSIGN cValor = TRIM(STRING(oldTab.dt-feriado,"99/99/9999")).
                END.
            END CASE.
        END.
    END CASE.
...
IF param-mn.sincroniza THEN DO:
    ...
    ELSE DO:
        ...
        ASSIGN lote-mov-campo.num-mov = iNum
               ...
               lote-mov-campo.chave   = YES
               lote-mov-campo.origem  = param-mn.maquina-local
               lote-mov-campo.antes   = TRIM(STRING(oldTab.dt-feriado,"99/99/9999")).
               lote-mov-campo.depois  = TRIM(STRING(newTab.dt-feriado,"99/99/9999")).
