...
&GLOBAL-DEFINE FrameFields  apontamento.qt-apontada apontamento.nr-pedido ~
                            ...
                            apontamento.cd-favorecido fav apontamento.reembolsavel ~
...
apontamento.hora-ini apontamento.nr-pedido apontamento.sequencia ~
apontamento.qt-apontada apontamento.cd-favorecido apontamento.nr-mov ~
...
DEFINE FRAME DEFAULT-FRAME
     ...
     fav AT ROW 4.5 COL 18 COLON-ALIGNED NO-LABEL WIDGET-ID 128 NO-TAB-STOP 
...
{func\pesq\lista_objeto.i &FRAME = {&FRAME-NAME}
                          ...
                          &ObjPai = apontamento.cd-favorecido}
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN apontamento.hora-fim = apontamento.hora-ini
           ...
           apontamento.ds-fav-apont = INPUT FRAME {&FRAME-NAME} fav.
...
APPLY "LEAVE" TO apontamento.cd-favorecido.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE apontamento THEN 
    DISPLAY apontamento.nome apontamento.data apontamento.hora-ini 
          apontamento.nr-pedido apontamento.sequencia apontamento.qt-apontada 
          apontamento.cd-favorecido apontamento.nr-mov apontamento.tipo-despesa 
          apontamento.reembolsavel apontamento.ds-apontamento 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE apontamento.nome apontamento.data apontamento.hora-ini 
         apontamento.nr-pedido apontamento.sequencia apontamento.qt-apontada 
         apontamento.cd-favorecido apontamento.nr-mov apontamento.tipo-despesa 
         apontamento.reembolsavel apontamento.ds-apontamento RECT-2 RECT-22 
         RECT-23 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
IF cObject = "fav" THEN DO:
    ...
    IF AVAIL favorecido THEN DO:
        ASSIGN apontamento.cd-favorecido:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(favorecido.cd-favorecido).
    END.
END.
...
OR pcActionBT = "UPDATE" THEN DO:
    ...
    IF AVAIL pedido-item THEN DO:
        ...
        FOR FIRST item NO-LOCK OF pedido-item,
            ...
            IF INPUT FRAME {&FRAME-NAME} fav = "" THEN DO:
