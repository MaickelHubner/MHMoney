...
&GLOBAL-DEFINE Filho         pedido-item-anexo
&GLOBAL-DEFINE LabelFilho    Anexos
&GLOBAL-DEFINE ParentFields  pedido-item.nr-pedido pedido-item.sequencia ~
                             pedido-item.cd-item
&GLOBAL-DEFINE KeyFields     pedido-item.nr-pedido pedido-item.sequencia
&GLOBAL-DEFINE CamposFilho   pedido-item-anexo.nr-anexo ~
                             pedido-item-anexo.ds-arquivo ~
...
&Scoped-Define ENABLED-OBJECTS br-page1 btVer btAdd-page1 btDel-page1 ~
...
pedido-item.cd-item item.ds-item 
&Scoped-define DISPLAYED-TABLES pedido-item item
...
DEFINE BUTTON btVer 
     ...
     LABEL "ver" 
     SIZE 6.14 BY 1.79 TOOLTIP "Visualizar Anexo".
...
DEFINE FRAME DEFAULT-FRAME
     ...
     btVer AT ROW 14.79 COL 89 WIDGET-ID 14
     ...
     pedido-item.nr-pedido AT ROW 4.5 COL 13 COLON-ALIGNED WIDGET-ID 8
          VIEW-AS FILL-IN 
          SIZE 6.86 BY .79
     pedido-item.sequencia AT ROW 4.5 COL 32 COLON-ALIGNED WIDGET-ID 18
          VIEW-AS FILL-IN 
          SIZE 5 BY .79
     pedido-item.cd-item AT ROW 4.5 COL 44 COLON-ALIGNED WIDGET-ID 16
          VIEW-AS FILL-IN 
          SIZE 6 BY .79
     item.ds-item AT ROW 4.5 COL 50 COLON-ALIGNED NO-LABEL WIDGET-ID 20
          VIEW-AS FILL-IN 
          SIZE 41 BY .79
...
/* SETTINGS FOR FILL-IN item.ds-item IN FRAME DEFAULT-FRAME
   NO-ENABLE                                                            */
...
DO:
    ...
    IF AVAIL pedido-item-anexo THEN DO:
        ASSIGN cArq = pedido-item-anexo.ds-arquivo.
    END.
...
ON CHOOSE OF btVer IN FRAME DEFAULT-FRAME /* ver */
DO:
    ...
    IF AVAIL pedido-item-anexo THEN DO:
        ...
        RUN ShellExecuteA(INPUT 0,
                          INPUT "open":u,
                          INPUT pedido-item-anexo.ds-arquivo,
                          INPUT "",
                          INPUT "",
                          INPUT 1,  /* normal mode */
                          OUTPUT iInstance).
        ...
        IF (iInstance = 31 OR 
            iInstance = 27) THEN DO:
           RUN ShellExecuteA (INPUT 0,
                              INPUT "open":u,
                              INPUT "rundll32.exe":u,
                              INPUT "shell32.dll,OpenAs_RunDLL ":u + pedido-item-anexo.ds-arquivo,
                              INPUT "",
                              INPUT 1,
                              OUTPUT iInstance).
        END.
...
IF NOT AVAIL {&Table} THEN DO:
    ...
    ASSIGN pedido-item.nr-pedido:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ?
           ...
           pedido-item.cd-item:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ?.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE pedido-item THEN 
    DISPLAY pedido-item.nr-pedido pedido-item.sequencia pedido-item.cd-item 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE br-page1 btVer btAdd-page1 btDel-page1 btExit btFirst btLast btNext 
         btPrev btUpdate-page1 btGoTo btZoom pedido-item.nr-pedido 
         pedido-item.sequencia pedido-item.cd-item RECT-5 RECT-6 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
