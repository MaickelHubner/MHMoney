...
&GLOBAL-DEFINE ParentFields  bens.cd-bens bens.ds-bens
&GLOBAL-DEFINE KeyFields     bens.cd-bens
&GLOBAL-DEFINE CamposFilho   fnId-tipo(mov-bens.id-tipo) @ cId-tipo ~
                             mov-conta.dt-mov ~
                             fnCd-conta(mov-bens.cd-conta) @ cCd-conta ~
                             mov-bens.cd-sequencia ~
                             mov-conta.de-valor
...
&Scoped-define INTERNAL-TABLES {&Filho} mov-conta bens
...
&Scoped-define QUERY-STRING-br-page1 FOR EACH {&Filho} OF {&Table} NO-LOCK, ~
           EACH mov-conta OF {&Filho}      BY {&Filho}.id-tipo      BY mov-conta.dt-mov INDEXED-REPOSITION. RUN controlButtons IN THIS-PROCEDURE
&Scoped-define OPEN-QUERY-br-page1 OPEN QUERY {&SELF-NAME} FOR EACH {&Filho} OF {&Table} NO-LOCK, ~
           EACH mov-conta OF {&Filho}      BY {&Filho}.id-tipo      BY mov-conta.dt-mov INDEXED-REPOSITION. RUN controlButtons IN THIS-PROCEDURE.
&Scoped-define TABLES-IN-QUERY-br-page1 {&Filho} mov-conta
&Scoped-define FIRST-TABLE-IN-QUERY-br-page1 {&Filho}
&Scoped-define SECOND-TABLE-IN-QUERY-br-page1 mov-conta
...
&Scoped-Define ENABLED-FIELDS bens.cd-bens bens.ds-bens 
...
&Scoped-Define DISPLAYED-FIELDS bens.cd-bens bens.ds-bens 
...
DEFINE FRAME DEFAULT-FRAME
     ...
     btAdd-page1 AT ROW 1.5 COL 56
     btDel-page1 AT ROW 1.5 COL 68.29
     ...
     btUpdate-page1 AT ROW 1.5 COL 62.14
     bens.cd-bens AT ROW 4.5 COL 21 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 5 BY .79
     bens.ds-bens AT ROW 4.5 COL 26.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 49 BY .79
...
IF NOT AVAIL {&Table} THEN DO:
    ...
    ASSIGN bens.cd-bens:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ?
           bens.ds-bens:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE bens THEN 
    DISPLAY bens.cd-bens bens.ds-bens 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE br-page1 btAdd-page1 btDel-page1 btExit btFirst btLast btNext btPrev 
         btUpdate-page1 bens.cd-bens bens.ds-bens btGoTo btZoom RECT-5 RECT-6 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
    ...
    CASE iCod:
        WHEN 1 THEN ASSIGN cReturn = "Aquisiá∆o".
        WHEN 2 THEN ASSIGN cReturn = "Valor Agregado".
        WHEN 3 THEN ASSIGN cReturn = "Manutená∆o".
